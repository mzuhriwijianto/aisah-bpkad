...
@section("contentheader_title")
	<a href="{{ url(config('laraadmin.adminRoute') . '/ajuan_penghapusanpbps') }}">Ajuan Penghapusan Pembantu PB</a> :
@endsection
@section("contentheader_description", $ajuan_penghapusanpbp->$view_col)
@section("section", "Ajuan Penghapusan Pembantu PB")
@section("section_url", url(config('laraadmin.adminRoute') . '/ajuan_penghapusanpbps'))
@section("sub_section", "Edit")

@section("htmlheader_title", "Ajuan Penghapusan PB Pembantu: ".$ajuan_penghapusanpbp->$view_col)
			...
			<div class="col-md-12">
				...
				<div class="col-md-3">
					<div class="alert alert-info">
						...
						<input class="form-control" placeholder="Enter No_ajuan" data-rule-maxlength="256" name="no_ajuan" type="text" value="{{ $ajuan_penghapusanpbp->no_ajuan }}" readonly>
						<label for="no_ajuan">Tgl ajuan :</label>
						<input class="form-control" placeholder="Enter Tgl_ajuan" name="tgl_ajuan" type="text" value="{{ date('d-m-Y', strtotime($ajuan_penghapusanpbp->tgl_ajuan)) }}" readonly>									
						<br>					
					</div>
					@if($ajuan_penghapusanpbp->validation_pb == '') OR ($ajuan_penghapusanpbp->validation_pb == 0) OR ($ajuan_penghapusanpbp->validation_pb == 99) OR ($ajuan_penghapusanpbp->validation_aset == 99)
						<div class="alert alert-danger">
				...
				<div class="col-md-3" style="border:#48B0F7; border-width:1px; border-style:solid;">
						<div class="col-md-12">
						...
						<div class="col-md-12">
							<br/>
							@if(($ajuan_penghapusanpbp->validation_pb == 0) OR ($ajuan_penghapusanpbp->validation_pb == 99) OR ($ajuan_penghapusanpbp->validation_aset == 99))
							<button id = "showbrg" class="btn btn-success btn-sm" data-toggle="modal" data-target="#addbrg" enabled>Show Barang</button>
							<button id = "showbrgbcd" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#addbrgbcd" enabled>Scan Barcode</button>							
							@else
							<button id = "showbrg" class="btn btn-success btn-sm" data-toggle="modal" data-target="#addbrg" disabled>Show Barang</button>
							<button id = "showbrgbcd" class="btn btn-info btn-sm" data-toggle="modal" data-target="#addbrgbcd" disabled>Scan Barcode</button>							
							@endif							
						</div>
						<div class="col-md-12">
						<br/>
				...
				<div class="col-md-6">
						...
						<div class="col-md-6">
							<div class="alert alert-dark" style="border:#48B0F7; border-width:1px; border-style:solid;">
								<label> Jenis Ajuan : </label> <i> {{ $ajuan_penghapusanpbp->jenis_ajuan }}</i>
								<hr/>
								...
								<input class="form-control" placeholder="Enter No_Surat" data-rule-maxlength="256" required="1" name="no_surat" type="text" value="{{ date('d-m-Y', strtotime($ajuan_penghapusanpbp->tgl_surat))}}" readonly>						
								...
								<div class="btn-group" role="group" aria-label="Basic example">	
										<button class="btn btn-success" onclick=" window.open('{{url('/storage/php_ppb/file/'.$ajnphp->filepdf)}}','_blank')">Preview Surat usulan</button>
								</div>																	
								@endforeach	
								@else
						...
						<div class="col-md-6">								
								...
								<h4> Proses</h4>
									@if($ajuan_penghapusanpbp->validation_pb == 0 )
										
										<form action = "/admin/ajuan_penghapusanpbps/ajukanpb/{{ $ajuan_penghapusanpbp->id }}" method="get" enctype="multipart/form-data">
											<input name="_token" type="hidden" value="{{ csrf_token() }}">																					
											<button type="submit" name="submit" class="btn btn-primary ">Ajukan Ke Pengurus Barang</button>										
										</form>
									@elseif($ajuan_penghapusanpbp->validation_pb == 99 )
										<form action = "/admin/ajuan_penghapusanpbps/ajukanpb/{{ $ajuan_penghapusanpbp->id }}" method="get" enctype="multipart/form-data">
											<button type="submit" class="btn btn-primary " data-dismiss="modal">Ajukan Ke Pengurus Barang</button>
										</form>
									@else
										<button type="button" class="btn btn-primary " data-dismiss="modal" disabled>Ajukan Ke Pengurus Barang</button>
									@endif
								</div>
						</div>
					...
					<div class="modal fade" id="addbrg" role="dialog" aria-labelledby="myModalLabel">
						<div class="modal-dialog modal-lg" role="document">
							<div class="modal-content">
								...
								<div class="form-group" >
									<h3>No Ajuan:</h3>
										<input type="text" id="ajuan" class="form-control" value="{{ $ajuan_penghapusanpbp->no_ajuan }}" readonly />
			...
			<tbody id = "pm1">
				...
				<tr>
					...
					<td>					
						...
						<button class="btn btn-success" onclick=" window.open('{{url('/storage/php_ppb/Image/'.$data_as->imgname)}}','_blank')">Preview</button>					
						<form action="/admin/ajuan_penghapusanpbps/upload-file/{{ $data_as->id }}" method="post" enctype="multipart/form-data">														
						<input name="_token" type="hidden" value="{{ csrf_token() }}">
							<div class="custom-file">
					...
					@if(($ajuan_penghapusanpbp->validation_pb == 0) OR ($ajuan_penghapusanpbp->validation_pb == 99) OR ($ajuan_penghapusanpbp->validation_aset == 99))
					 ...
					 <button id = "{{ $data_as->id }}" 
							   href="{{ url(config('laraadmin.adminRoute') . '/delbrgpbp/'.$data_as->id)}}" 
			...
			<tbody id = "pm1">
				...
				<tr>
					...
					<td>					
						...
						<button class="btn btn-success" onclick=" window.open('{{url('/storage/php_ppb/Image/'.$data_bs->imgname)}}','_blank')">Preview</button>					
						<form action="/admin/ajuan_penghapusanpbps/upload-file/{{ $data_bs->id }}" method="post" enctype="multipart/form-data">														
						<input name="_token" type="hidden" value="{{ csrf_token() }}">
							<div class="custom-file">
								<input type="file" name="file" class="custom-image-input" id="chooseFile">
							</div>
			...
			<tbody id = "pm1">
				...
				<tr>
					...
					<td>					
						...
						<button class="btn btn-success" onclick=" window.open('{{url('/storage/php_ppb/Image/'.$data_cs->imgname)}}','_blank')">Preview</button>					
						<form action="/admin/ajuan_penghapusanpbps/upload-file/{{ $data_cs->id }}" method="post" enctype="multipart/form-data">														
						<input name="_token" type="hidden" value="{{ csrf_token() }}">
							<div class="custom-file">
								<input type="file" name="file" class="custom-image-input" id="chooseFile">
							</div>
			...
			<tbody id = "pm1">
				...
				<tr>
					...
					<td>			
						...
						<button class="btn btn-success" onclick=" window.open('{{url('/storage/php_ppb/Image/'.$data_ds->imgname)}}','_blank')">Preview</button>					
						<form action="/admin/ajuan_penghapusanpbps/upload-file/{{ $data_ds->id }}" method="post" enctype="multipart/form-data">														
						<input name="_token" type="hidden" value="{{ csrf_token() }}">
							<div class="custom-file">
								<input type="file" name="file" class="custom-image-input" id="chooseFile">
							</div>
					...
					@if(($ajuan_penghapusanpbp->validation_pb == 0) OR ($ajuan_penghapusanpbp->validation_pb == 99) OR ($ajuan_penghapusanpbp->validation_aset == 99))
					 <button id = "{{ $data_ds->id }}" 
							   href="{{ url(config('laraadmin.adminRoute') . '/delbrgpbp/'.$data_ds->id)}}" 
							   class="btn btn-danger" type="button" onclick = "delidpemda(this.id)" 
		...
		<div class="box-body table-responsive">
			...
			<tbody id = "pm1">
				...
				<tr>
					...
					<td>
					@if(($ajuan_penghapusanpbp->validation_pb == 0) OR ($ajuan_penghapusanpbp->validation_pb == 99) OR ($ajuan_penghapusanpbp->validation_aset == 99))
					 <button id = "{{ $data_es->id }}" 
							   href="{{ url(config('laraadmin.adminRoute') . '/delbrgpbp/'.$data_es->id)}}" 
							   class="btn btn-danger" type="button" onclick = "delidpemda(this.id)" 
			...
			<tbody id = "pm1">
				...
				<tr>
					...
					<td>					
						...
						<button class="btn btn-success" onclick=" window.open('{{url('/storage/php_ppb/Image/'.$data_fs->imgname)}}','_blank')">Preview</button>					
						<form action="/admin/ajuan_penghapusanpbps/upload-file/{{ $data_fs->id }}" method="post" enctype="multipart/form-data">														
						<input name="_token" type="hidden" value="{{ csrf_token() }}">
							<div class="custom-file">
								<input type="file" name="file" class="custom-image-input" id="chooseFile">
							</div>
...
<script>
$(function () {
	$("#ajuan_penghapusanpbp-edit-form").validate({
...
jQuery(document).ready(function (){
            jQuery('select[name="rek0"]').on('change',function(){
               ...
               if(rekening0){
                  jQuery.ajax({
                     url : '/admin/ajuan_penghapusanpbp_rek1/' +rekening0,
                     type : "GET",
                     dataType : "json",
...
jQuery(document).ready(function (){
            jQuery('select[name="rek1"]').on('change',function(){
               ...
               if(rekening1){				  
                  jQuery.ajax({
                     url : '/admin/ajuan_penghapusanpbp_rek2/' +rekening0+'/'+rekening1,
                     type : "GET",
                     dataType : "json",
                     success:function(data)
                     {
	...
	jQuery(document).ready(function () {						
		jQuery(document).on('click', '#showbrg', function(e){
            ...
            function fetch_customer_data(query = '') {
                jQuery.ajax({
					url:'/admin/showbarangphppbp/'+{{ $ajuan_penghapusanpbp->id }}+'/'+rekening1+'/'+rekening2+'/'+rekening3+'/'+rekening4+'/'+kondisi,
                    method: 'GET',
                    data: {query: query},
                    dataType: 'json',
                    success: function (data) {
...
function getidpemda(clicked_id){
			...
			var rekening0 = $("#rek0 option:selected").val();
			var kondisi = $("#kond option:selected").val();
			var loc = '/admin/showbarangphppbp/'+{{ $ajuan_penghapusanpbp->id }}+'/'+rekening1+'/'+rekening2;
			var noajuan =  {{ $ajuan_penghapusanpbp->id }};
			var idpemdas =  clicked_id;		
			
					   $.ajax({
						//url : '/admin/addbrgpbp',//+{{ $ajuan_penghapusanpbp->id }}+'/'+idpemdas,
						url : '/admin/addbrgpbp/'+{{ $ajuan_penghapusanpbp->id }}+'/'+idpemdas+'/'+rekening0+'/'+rekening1+'/'+rekening2+'/'+rekening3+'/'+rekening4,
						headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
						method: 'POST',						
						data: {
								  "_token": "{{ csrf_token() }}",
								  "no_ajuan": {{ $ajuan_penghapusanpbp->id }},
								  "idpemda": idpemdas,
								  "jenis_ajuan": "PPB",
								  "kd_aset8" : 1,
								  "kd_aset80" : rekening0,
...
var idpemda = document.getElementById('myInput').value;
    $.ajax({
						url : '/admin/addbrgpbp/'+{{ $ajuan_penghapusanpbp->id }}+'/'+idpemda,
						headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
						method: 'POST',						
						data: {
								  "_token": "{{ csrf_token() }}",
