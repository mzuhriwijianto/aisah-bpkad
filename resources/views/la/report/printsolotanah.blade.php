...
<div class="divTable unstyledTable">
    <table class="minimalistBlack" style="width: 100%;" align = "center">
		...
		<tr id = "header">
				...
				<td style="width: 10%;">Sert File</td>
				<td style="width: 5%;">Foto Tanah</td>
		...
		@foreach ($tanahs as $index =>$tanaharray)
			...
			<td style="text-align:left;"> {{ date('d-m-Y', strtotime($tanaharray->tgl_perolehan)) }}</td>
			<td style="text-align:left;"> {{ $tanaharray->No_Register }}</td>
			<td style="text-align:left;"> {{ $tanaharray->Sertifikat_Nomor }}</td>
			<td style="text-align:left;"> {{ date('d-m-Y', strtotime($tanaharray->Sertifikat_tanggal)) }}</td>
			<td style="text-align:left;"> {{ $tanaharray->hak_tanah}}</td>
			...
			<td style="text-align:center;">{{ (( $tanaharray->idsert == '' OR $tanaharray->idsert == 0) ? 'X' : 'V') }}</td>
			<td style="text-align:center;">{{ (( $tanaharray->idphoto == '' OR $tanaharray->idphoto == 0) ? 'X' : 'V') }}</td>
