	...
	<div class="divTable unstyledTable">
		<div class="divTableBody">
			<div class="divTableRow">
				...
				<div class="divTableCell3" align="left" style = "width: 45%;">
					<pre style = "margin: 25px 0px 0px 1px;">
...
<div class="divTable unstyledTable">
    <table class="minimalistBlack" style="width: 100%;" align = "center">
		...
		<tr id = "header">
				...
				<td style="width: 8%;">NO. IMB LAMA</td>
				<td style="width: 8%;">NO. IMB BARU</td>
                <td style="width: 5%;">TGL PEROLEHAN</td>
                 <td style="width: 3%;">NO. REG</td>
				<td style="width: 10%;">ASAL-USUL</td>
				<td style="width: 10%;">PENGGUNAAN</td>
				<td style="width: 10%;">PEMANFAATAN</td>
                 <td style="width: 10%;">LOKASI</td>
                 <td style="width: 10%;">HARGA</td>
                 <td style="width: 10%;">TGL TERBIT IMB</td>
				<td style="width: 10%;">TGL AKHIR IMB</td>
				<td style="width: 5%;">DOKUMEN IMB</td>
		...
		@foreach ($gbs as $index =>$gbarray)
			...
			<td style="text-align:left;"> {{ $gbarray->no_imb_lama }}</td>
			<td style="text-align:left;"> {{ $gbarray->no_imb_baru }}</td>
            <td style="text-align:left;"> {{ date('d-m-Y', strtotime($gbarray->tgl_perolehan)) }}</td>
             <td style="text-align:left;"> {{ $gbarray->no_register }}</td>
           <td style="text-align:left;"> {{ $gbarray->asal_usul }}</td>
           <td style="text-align:left;"> {{ $gbarray->penggunaan}}</td>
           <td style="text-align:center;"> {{ $gbarray->pemanfaatan }}</td>
           <td style="text-align:center;"> {{ $gbarray->lokasi }}</td>
           <td style="text-align:center;"> {{ $gbarray->harga }}</td>
			<td style="text-align:left;"> {{ date('d-m-Y', strtotime($gbarray->tgl_awal_imb)) }}</td>
            <td style="text-align:left;"> {{ date('d-m-Y', strtotime($gbarray->tgl_akhir_imb)) }}</td>
			<td style="text-align:center;">{{ (( $gbarray->dokumen == '' OR $gbarray->dokumen == 0) ? 'X' : 'V') }}</td>
			</tr>
		@endforeach
		</tbody>
		</tr>
