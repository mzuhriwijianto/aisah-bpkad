...
class UploadsController extends Controller
{
	...
	public function index(){
		...
		//$module = Module::get('Uploads');
		$module = Module::get('Uploads');//->where('Uploads.user_id','=',Auth::user()->id);
    ...
    public function get_file($hash, $name)
    {
        ...
        //if($upload->public || Entrust::hasRole('SUPER_ADMIN') || Auth::user()->id == $upload->user_id) {
          if(($upload->public == true || $upload->user_id == Auth::user()->id ) || ($kdbidang == 99 || $kdbidang == 0 )) {   
            $path = $upload->path;
            ...
            return $response;
		}elseif(($upload->public == false) || ($upload->user_id == Auth::user()->id)||($kdbidang == 99) || ($kdbidang == 0 )){
			$path = $upload->path;
    ...
    public function update_caption()
    {
        if(Module::hasAccess("Uploads", "edit")) {
			...
			if(isset($upload->id)) {
				...
				if(($upload->public == true || $upload->user_id == Auth::user()->id ) || Entrust::hasRole('SUPER_ADMIN')) {
    ...
    public function delete_file()
    {
        if(Module::hasAccess("Uploads", "delete")) {
			...
			if(isset($upload->id)) {
				...
				}else if(($upload->public == false || $upload->user_id == Auth::user()->id ) || (Entrust::hasRole('SUPER_ADMIN'))) {
					$upload->delete();
